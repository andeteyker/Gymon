...
export interface MonsterStats {
 ...
 performance:Record<PerformanceStat,number>;
}

export interface Monster {
 id:string; nickname:string; starterFamily:string; formId:string;
 level:number; xp:number; tokens:number; stats:MonsterStats; activeMoveIds:string[];
}

export interface Move {
 id:string; name:string; power:number; accuracy:number; priority:number; energyCost:number;
 requiredMuscle?:MuscleGroup; requiredLevel?:number; evolutionExclusive?:string;
}

export interface Evolution {
 id:string; name:string; stage:number; levelCap:number;
 previousForms:string[]; nextForms:string[];
 requirements:Record<string,number>; exclusiveMoveIds:string[];
}
